...
KERNEL="owrt-kernel.bin"
ROOTFS="owrt-rootfs.img"
BASEURL="https://downloads.openwrt.org/releases/${OWRT_VER}/targets/armsr/armv8"
KERNEL_URL="${BASEURL}/openwrt-${OWRT_VER}-armsr-armv8-generic-kernel.bin"
ROOTFS_URL="${BASEURL}/openwrt-${OWRT_VER}-armsr-armv8-generic-ext4-rootfs.img.gz"
HTTP_PORT=18080
...
for dep in qemu-system-aarch64 expect python3; do
    command -v "$dep" >/dev/null 2>&1 \
        || die "не найден: $dep  (macOS: brew install qemu)"
done
...
if [ ! -f "$KERNEL" ]; then
    info "Скачиваю ядро..."
    curl -L --progress-bar -o "$KERNEL" "$KERNEL_URL" \
        || die "не удалось скачать ядро"
fi

if [ ! -f "$ROOTFS" ]; then
    info "Скачиваю rootfs..."
    curl -L --progress-bar -o "${ROOTFS}.gz" "$ROOTFS_URL" \
        || die "не удалось скачать rootfs"
    info "Распаковываю rootfs..."
    gunzip "${ROOTFS}.gz"
    qemu-img resize -f raw "$ROOTFS" +64M >/dev/null
fi
...
expect {
    -re "(?i)please press enter" { }
    timeout { puts "\nTIMEOUT: OpenWrt не загрузился"; exit 1 }
}
after 500
send "\r"
after 2000
...
expect {
    "root@" { }
    timeout { puts "\nTIMEOUT: нет приглашения оболочки"; exit 1 }
}
after 300
...
expect {
    "__GOT__" { }
    timeout { puts "\nTIMEOUT: не удалось скачать скрипт"; exit 1 }
}
expect "root@"
...
expect {
    "Итог:" { }
    timeout { puts "\nTIMEOUT: тесты не завершились"; exit 1 }
}
expect "root@"
