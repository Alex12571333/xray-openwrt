...
select_best_servers() {
    ...
    local alive="${WORK_DIR}/alive.txt" scored="${WORK_DIR}/scored.txt"
    > "$alive"; > "$scored"
    ...
    while IFS= read -r line && [ "$tested" -lt "$test_max" ]; do
        ...
        # Быстрый парсинг host:port без вызова parse_vless
    ...
    local found
    found=$(wc -l < "$scored" | tr -d ' ')

    if [ "$found" -gt 0 ]; then
        printf '>>> Доступно: %s — беру топ-%s по латентности\n' "$found" "$want" >&2
        sort -n "$scored" | head -"$want" | cut -f2-
    else
        printf 'ПРЕДУПРЕЖДЕНИЕ: нет доступных серверов в первых %s — беру первые %s без проверки\n' \
            "$test_max" "$want" >&2
        head -"$want" "$input"
    fi
}
